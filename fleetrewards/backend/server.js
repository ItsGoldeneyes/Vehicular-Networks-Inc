...
async function synchronize_activites(id) {
  try {
    const names = await client.query(`SELECT first_name, last_name FROM users WHERE id = ${id}`)
    const g5_activities = axios.post(`https://backend-group5.up.railway.app/get-forms`, {username:(names.rows[0].first_name + " " + names.rows[0].last_name)});
    const our_activity_ids = await client.query(`SELECT activity_id FROM activities WHERE user_id = ${userId}`);
    const activity_ids = our_activity_ids.map((activity) => activity.activity_id);

    g5_activities = g5_activities.filter((activity) => !activity_ids.includes(activity.form.form_id));
    g5_activities.array.forEach(element => {
      client.query(`INSERT INTO activities (activity_id, user_id, type, points_change) VALUES (${element.form.form_id}, ${element.form.id}, ${element.form.type}, ${element.form.points})`)
    });
